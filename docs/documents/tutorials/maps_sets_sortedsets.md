...
The other day I came across a question about how to model popuplar Redis datatypes using COX Edge. COX Edge is a geo-replicated multi-model realtime database. So modelling Redis basic datatypes in COX Edge can be done easily. 
...
All of the above can be represented in COX Edge.
...
Sets are basically an unordered collection of unique Strings. Sets can be easily modelled in COX Edge as a collection of documents.
...
Lists are similar to sets except the values are not unique. Lists can be easily modelled in COX Edge as a collection of documents like `Sets`. Use a different field instead of `_key` field. Create an index on that field with `unique=false`.
...
SortedSets can be easily modelled in COX Edge as a collection of documents. Collections are a general-purpose storage mechanism and they are not limited to storing just scores.
